...
import { updateNoticeUrl } from "@/utils/routes"
import { auth } from "auth"
import { revalidateTag } from "next/cache"

export default async function patchUpdateNotice(state: State, formData: unknown) {
    ...
    try {
        ...
        if (!parsedFormData.data.id) return ResponseError("Notice ID is required.")

        const URL = updateNoticeUrl(parsedFormData.data.id)
