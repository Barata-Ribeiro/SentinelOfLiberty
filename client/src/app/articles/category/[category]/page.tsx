...
export default async function ArticlesCategoryPageProps({ params, searchParams }: Readonly<ArticlesCategoryPageProps>) {
    ...
    const articlesState = await getAllPublicArticlesByCategoryPaginated({
    ...
    const descriptionMessage =
        pagination.page.totalElements > 0 ? "Enjoy reading them!" : "Wait for our authors to publish some!"
        ...
        <div className="container" aria-labelledby="page-title" aria-describedby="page-description">
            <header className="mt-4 max-w-2xl sm:mt-8">
                <h1
                    ...
                    Articles for {decodedCategory}
                </h1>
                <p id="page-description" className="text-shadow-600 mt-2 text-lg/8">
                    There are currently {pagination.page.totalElements} article(s) available under the {decodedCategory}{" "}
                    category. {descriptionMessage}
                </p>
            </header>
