...
export default function FormTextarea({ label, ...props }: FormTextareaProps) {
    return (
        <Field className="relative w-full min-w-[200px]" disabled={ props.disabled }>
            <Textarea
                { ...props }
                className="peer h-full min-h-[100px] w-full resize-none rounded-[7px] border border-stone-200 border-t-transparent bg-transparent px-3 py-2.5 font-sans text-sm font-normal text-stone-700 shadow-xs outline-0 transition-all placeholder-shown:border placeholder-shown:border-stone-200 placeholder-shown:border-t-stone-200 focus:border-2 focus:border-stone-900 focus:border-t-transparent focus:ring-1 focus:shadow-sm focus:ring-stone-400/50 focus:outline-hidden disabled:resize-none disabled:border-0 disabled:bg-stone-50"
